# -*- coding: utf-8 -*-
"""
v8.0 — Mon Portefeuille IA stricte & Bench Total/PEA/CTO
- Structure type V6.9 conservée (recherche, convertisseur, tableau éditable)
- IA stricte (decisions conservatrices, held=True)
- Décision IA + 🎯 Priorité + Proximité + emojis
- Synthèse PEA/CTO/Total
- Benchmark comparatif (3 messages: Total, PEA, CTO)
...
st.title("💼 Mon Portefeuille — IA stricte & benchmark")
...
with st.expander("🔁 Convertisseur LS Exchange → Yahoo"):
    ...
    with cC:
        if st.button("✅ Enregistrer mapping"):
...
def sty_dec(v):
    s = str(v)
    if "Acheter" in s:    return "background-color:rgba(0,180,0,0.18);font-weight:600;"
    if "Vendre" in s:     return "background-color:rgba(255,0,0,0.18);font-weight:600;"
    if "Surveiller" in s: return "background-color:rgba(0,90,255,0.18);font-weight:600;"
    if "Garder" in s:     return "background-color:rgba(0,120,255,0.12);"
    return ""

def sty_priority(v):
    s = str(v)
    if "Vendre" in s:  return "background-color:#ffebee;color:#b71c1c;font-weight:600;"
    if "Alléger" in s: return "background-color:#fff8e1;color:#a67c00;font-weight:600;"
    if "Couper" in s:  return "background-color:#ffe0e0;color:#a80000;font-weight:600;"
...
# NOTES
# ==============================
st.caption("💡 Les décisions IA sont **strictes** (mode ‘held=True’), et tiennent compte du profil via lib.get_profile_params().")
